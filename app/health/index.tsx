...
export default function HealthMain() {
  ...
  const pickImage = async () => {
    let result = await ImagePicker.launchImageLibraryAsync({
      ...
      allowsEditing: true,
      quality: 0.8, // Chất lượng cao chút để AI đọc
      base64: true, // <--- QUAN TRỌNG: Lấy base64 để gửi AI
    });

    if (!result.canceled && result.assets[0].base64) {
      // Truyền cả uri và base64 sang trang kết quả
      router.push({
        pathname: "/health/result",
        params: {
          imageUri: result.assets[0].uri,
          base64: result.assets[0].base64, // Truyền dữ liệu ảnh
        },
      });
    }
  };
